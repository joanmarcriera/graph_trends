...
tick_positions = [monthly_data.index[monthly_data['YearMonth'] == pd.Period(year)].tolist()[0] for year in even_years if not monthly_data.index[monthly_data['YearMonth'] == pd.Period(year)].empty]

plt.xticks(tick_positions, [str(year) for year in range(2008, 2025, 2)], rotation=45)
